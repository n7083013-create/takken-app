...
describe('useProgressStore - first_question_answered (アクティベーション計測)', () => {
  ...
  // [flaky 対策] jest では trackFirstQuestionAnsweredActivation() が
  // isJestRuntime() ガードにより *同期的な* require 経路を通る
  // (動的 import('../services/analytics') は本番のみ)。
  // よって trackEvent は recordAnswer() の中で同期的に呼ばれており、
  // `await new Promise(r => setImmediate(r))` での flush は不要だった。
  // この余計な await は CPU が逼迫した worker 上で jest の 5s タイムアウトに
  // 引っかかる suspension point になり得たため、同期テストに是正する。

  it('初回正解時に trackEvent("first_question_answered") が発火する', () => {
  ...
  it('2回目の正解では発火しない (totalCorrect が 0 から始まらないため)', () => {
    const { trackEvent } = require('../../services/analytics');
    useProgressStore.getState().recordAnswer('q1', 'kenri', true);
...
// ============================================================
// 合格エンジン連携: getPreSleepReview (夜版) / 締切 cap の store 配線
// ============================================================
describe('useProgressStore - getPreSleepReview (夜版差別化)', () => {
  beforeEach(() => {
    useProgressStore.getState().resetProgress();
    jest.clearAllMocks();
  });

  it('新規 (未解答=ブックマークのみ) を就寝前候補に含めない', () => {
    // q-fresh は未解答だがブックマークで progress に存在 (attempts=0)
    useProgressStore.getState().toggleBookmark('q-fresh');
    // q-done は解答済み
    useProgressStore.getState().recordAnswer('q-done', 'kenri', true, 'low');

    const ids = useProgressStore.getState().getPreSleepReview(5);
    expect(ids).not.toContain('q-fresh');
    expect(ids).toContain('q-done');
  });

  it('その日間違えた問題を最優先で再露出する', () => {
    // 多数の「普通の復習対象」を作る
    for (let i = 0; i < 6; i++) {
      useProgressStore.getState().recordAnswer(`q-ok-${i}`, 'kenri', true, 'high');
    }
    // 当日に間違えた問題 (correctStreak=0, lastAttemptAt=今日)
    useProgressStore.getState().recordAnswer('q-wrong', 'takkengyoho', false, 'low');

    const ids = useProgressStore.getState().getPreSleepReview(5);
    expect(ids[0]).toBe('q-wrong');
  });
});

describe('useProgressStore - recordAnswer 締切 cap 配線', () => {
  beforeEach(() => {
    useProgressStore.getState().resetProgress();
    jest.clearAllMocks();
  });

  it('正解を重ねても nextReviewAt が試験日を飛び越えない (interval が残り日数以内)', () => {
    const { daysUntilTakkenExam } = require('../../constants/exam');
    const daysToExam: number | null = daysUntilTakkenExam();
    // 試験日が取得できる前提 (宅建は固定日)。null の場合は cap 対象外なのでスキップ。
    if (daysToExam == null) return;

    // 高確信で連続正解を重ね、本来なら interval が試験日を超える状況を作る
    for (let i = 0; i < 8; i++) {
      useProgressStore.getState().recordAnswer('q-cap', 'kenri', true, 'high');
    }
    const p = useProgressStore.getState().getProgress('q-cap')!;
    // interval は試験前日まで (= 残り日数-1) で頭打ち。少なくとも残り日数は超えない。
    expect(p.interval).toBeLessThanOrEqual(daysToExam);
  });
});
